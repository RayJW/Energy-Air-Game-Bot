...
function total(){
    if(document.getElementsByClassName("mobile-padding-question")[0] === undefined){ //Fragen Beantworten
        //War eine Antwort falsch?
        if(document.getElementsByClassName("btn btn-primary game-button btn-lg")[0] === undefined){
            //Code für weiteres vorgehen --> auswähen um was gespielt werden soll.
        }else{
        ...
        if (questionEls[0].innerText == "10 / 10"){
            answer()
            document.getElementsByClassName("btn btn-primary game-button game-button-slot")[0].click(); //Hier soll man dann auf die Auswahl kommen um das Ticket zu gewinnen
        }
        else{
            answer()
            location.reload();
